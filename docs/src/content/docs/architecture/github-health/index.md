...
> **What this page is.** A map from "what could go wrong with a fetch" to "what the popup says, what storage commits, and how recovery happens." This page is the hub. The two children, [List Trust and Suspect Lists](./list-trust/) and [Outage Banner and Statuspage](./outage-banner/), unpack the integrity layer and the popup banner respectively.
...
In other words, "your assigned list went to zero" is not by itself an outage. It is most often the legitimate "I cleared my queue" steady state. The integrity layer waits for confirmation before believing it. The full state machine lives on [List Trust and Suspect Lists](./list-trust/).
...
The full client contract (cache TTL, fail-open behaviour, `bypassCache` semantics, the rare component-rename fallback) lives on [Outage Banner and Statuspage](./outage-banner/).
...
- [List Trust and Suspect Lists](./list-trust/): the integrity layer that decides whether a fresh fetch is allowed to replace the stored baseline, and the source of every `pr_component_degraded` and `pr_list_churn` signal.
- [Outage Banner and Statuspage](./outage-banner/): the popup-side contract. Reason-keyed copy, when the Statuspage link shows, the "Last check (kept your cached list)" subline.
- [The Parser Waterfall](../parser-waterfall/): the other side of the "is this an outage or a DOM change?" fork.
- [The Service Worker Lifecycle](../service-worker-lifecycle/): why `HealthStatusService` rehydrates its in-memory mirror on every wake.
