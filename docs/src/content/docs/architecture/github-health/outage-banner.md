---
title: Outage Banner and Statuspage
...
[GitHubStatusClient](https://github.com/dragosdev-code/pullwatch/blob/main/extension/common/github-status-client.ts) wraps the `summary.json` endpoint at `https://www.githubstatus.com/api/v2/summary.json`. The contract:
...
[src/hooks/use-github-status-snapshot.ts](https://github.com/dragosdev-code/pullwatch/blob/main/src/hooks/use-github-status-snapshot.ts) mirrors `STORAGE_KEY_GITHUB_STATUS_CACHE` into local React state. It validates the parsed shape (both `prComponentStatus` and `globalIndicator` must belong to known unions) and returns `null` for a missing or malformed cache. The two-minute staleness on the cache is deliberate: this snapshot is used to gate an informational link, and the alternative (always-on or always-off) is strictly worse.

### `useGitHubOutage`

[src/hooks/use-github-outage.ts](https://github.com/dragosdev-code/pullwatch/blob/main/src/hooks/use-github-outage.ts) mirrors `STORAGE_KEY_GITHUB_OUTAGE` plus `STORAGE_KEY_LAST_UNTRUSTED_FETCH_AT`, and also subscribes to the `githubOutage{Detected,Cleared}` runtime broadcasts. Two parsing rules are worth calling out:
