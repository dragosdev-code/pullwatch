...
| ------------------ | ------------------------------------------------------- | ---------------------------------------------- | ----------------------------------------------------------------- |
| 0. Status guard    | `tick`                                                  | `status !== 'playing'` returns early           | —                                                                 |
| 0. Hit-stop guard  | `tick`                                                  | `now < hitStopUntil` returns early             | —                                                                 |
...
**Do not split phase ownership across multiple top-level mutators.** Phases would be free to reorder relative to each other, and the `tick_ordering` suite enforces the order above precisely because that ordering is load-bearing for several behaviours.
