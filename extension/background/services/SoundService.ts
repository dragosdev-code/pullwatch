...
export class SoundService implements ISoundService {
   ...
   */
  async playSound(soundFile: string, volume = 1.0): Promise<void> {
    try {
      this.debugService.log(`[SoundService] Playing sound: ${soundFile}, volume: ${volume}`);
      await this.ensureOffscreenDocument();

      // Send message to the offscreen document for audio playback
      chrome.runtime.sendMessage(
        { action: EVENT_PLAY_SOUND, payload: { sound: soundFile, volume } },
        (response) => {
          if (chrome.runtime.lastError) {
            this.debugService.error(
              '[SoundService] Error sending play sound message to offscreen:',
      ...
      this.debugService.error('[SoundService] Error playing sound:', error);
      throw error;
    }
  }
  ...
  async stopAllSounds(): Promise<void> {
    try {
      this.debugService.log('[SoundService] Stopping all sounds');

      if (await this.hasOffscreenDocument()) {
        chrome.runtime.sendMessage({ action: 'stopAllSounds' }, (response) => {
          if (chrome.runtime.lastError) {
            this.debugService.error(
              '[SoundService] Error sending stop sounds message to offscreen:',
              chrome.runtime.lastError.message
            );
          } else {
            this.debugService.log(
              '[SoundService] Stop sounds message sent to offscreen, response:',
              response
            );
          }
        });
      ...
      this.debugService.error('[SoundService] Error stopping sounds:', error);
      throw error;
    }
  }
  ...
  async isSoundEnabled(): Promise<boolean> {
    // This would typically check settings through storage service
  ...
  async setSoundEnabled(enabled: boolean): Promise<void> {
    this.debugService.log(`[SoundService] Sound enabled status changed to: ${enabled}`);
    // This would typically update settings through storage service
  }
  ...
  async setVolume(volume: number): Promise<void> {
    this.debugService.log(`[SoundService] Volume changed to: ${volume}`);
    // This would typically send a message to offscreen document to update volume
    if (await this.hasOffscreenDocument()) {
      chrome.runtime.sendMessage({ action: 'setVolume', payload: { volume } }, (response) => {
        if (chrome.runtime.lastError) {
          this.debugService.error(
            '[SoundService] Error sending volume change message to offscreen:',
            chrome.runtime.lastError.message
          );
        } else {
          this.debugService.log(
            '[SoundService] Volume change message sent to offscreen, response:',
            response
          );
        }
      });
  ...
  async getVolume(): Promise<number> {
    // This would typically query the offscreen document for current volume
