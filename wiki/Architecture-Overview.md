...
| `HealthStatusService` | [HealthStatusService.ts](../extension/background/services/HealthStatusService.ts) | Summarises the health of the last fetch (ok, rate limited, signed out, parser failed) into a single value. |
...
| ------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------- |
| `chrome.storage.local`   | PR lists (`github_assigned_prs`, `github_merged_prs`, `github_authored_prs`), route hint, rate limit state, parser pattern registry, viewer identity for account swap detection, install gate flags, custom sound metadata. | No. This device only.      |
