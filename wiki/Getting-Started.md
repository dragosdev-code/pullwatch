...
Everything below lives in [package.json](../package.json). You will reach for these during day to day work.
...
| `npm run canary:test`                            | Runs the [canary suite](The-Canary-Monitor) locally using the current parser patterns.                                                                                                                                |
...
All permissions are declared in [public/manifest.json](../public/manifest.json). Each one exists for a single narrow reason. Chrome asks you to approve them at install time, and you deserve a plain English answer for every single one.
...
| `offscreen`     | Manifest V3 service workers cannot play audio. The [offscreen document](Notifications-and-Sound) is used to play notification sounds and nothing else. | Render any user facing UI in an offscreen document.                                                                      |

### Host permissions

Host permissions control which origins Pullwatch is allowed to send HTTP requests to. Pullwatch declares exactly three, and each one is narrow.
...
You can verify the outbound destinations at any time from the browser's DevTools network tab on the service worker. There is no fourth host. No telemetry, no analytics, no third party SDK.
...
If the popup stays empty, the [Onboarding and Session Gates](Onboarding-and-Session-Gates) page covers the three "we could not talk to GitHub" states and how Pullwatch tells you which one you are in.
...
- **You are interested in how Pullwatch survives Chrome killing the service worker every 30 seconds:** [The Service Worker Lifecycle](The-Service-Worker-Lifecycle).
- **You are curious how the UI feels instant even on a cold open:** [Data Hydration and Storage](Data-Hydration-and-Storage).
